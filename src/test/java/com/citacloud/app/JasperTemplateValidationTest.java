    ...
    void compilaPlantillaDeCierreDeCaja() {
        assertDoesNotThrow(() -> {
            try (var cierre = new ClassPathResource("reportes/cierre-caja.jrxml").getInputStream()) {
                ...
                parametros.put("DIRECCION", "Av. Principal 123"); parametros.put("CAJA", "Caja principal"); parametros.put("USUARIO", "Administrador"); parametros.put("DESGLOSE", "EFECTIVO: RD$ 100.00");
                ...
                List<Map<String, ?>> movimientos = List.of(
                        Map.of("hora", "08:57", "tipo", "Ingreso manual", "referencia", "MOV-000013", "metodo", "EFECTIVO", "entrada", new java.math.BigDecimal("1000.00"), "salida", java.math.BigDecimal.ZERO),
                        Map.of("hora", "09:05", "tipo", "Egreso manual", "referencia", "MOV-000016", "metodo", "TRANSFERENCIA", "entrada", java.math.BigDecimal.ZERO, "salida", new java.math.BigDecimal("200.00")),
                        Map.of("hora", "09:10", "tipo", "Cobro", "referencia", "PAG-000008", "metodo", "TARJETA", "entrada", new java.math.BigDecimal("500.00"), "salida", java.math.BigDecimal.ZERO));
                var documento = JasperFillManager.fillReport(reporte, parametros, new JRMapCollectionDataSource(movimientos));
                byte[] pdf = JasperExportManager.exportReportToPdf(documento); assertTrue(pdf.length > 1_000);
                Path salida = Path.of("output", "pdf", "cierre-caja-ejemplo.pdf"); Files.createDirectories(salida.getParent()); Files.write(salida, pdf);
            }
        });
    }
}
