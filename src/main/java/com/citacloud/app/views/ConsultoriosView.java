package com.citacloud.app.views;

import com.citacloud.app.models.Consultorio;
import com.citacloud.app.models.Sucursal;
import com.citacloud.app.security.AuthService;
import com.citacloud.app.security.TenantUserDetails;
import com.citacloud.app.services.ConsultorioService;
import com.citacloud.app.services.SucursalService;
import com.vaadin.flow.component.button.Button;
import com.vaadin.flow.component.button.ButtonVariant;
import com.vaadin.flow.component.combobox.ComboBox;
import com.vaadin.flow.component.dialog.Dialog;
import com.vaadin.flow.component.formlayout.FormLayout;
import com.vaadin.flow.component.grid.Grid;
import com.vaadin.flow.component.html.H2;
import com.vaadin.flow.component.html.Span;
import com.vaadin.flow.component.icon.VaadinIcon;
import com.vaadin.flow.component.notification.Notification;
import com.vaadin.flow.component.orderedlayout.FlexComponent;
import com.vaadin.flow.component.orderedlayout.HorizontalLayout;
import com.vaadin.flow.component.orderedlayout.VerticalLayout;
import com.vaadin.flow.component.textfield.TextField;
import com.vaadin.flow.router.PageTitle;
import com.vaadin.flow.router.Route;
import jakarta.annotation.security.PermitAll;

import java.util.ArrayList;
import java.util.List;
import java.util.UUID;

@Route(value = "consultorios", layout = MainLayout.class)
@PageTitle("Consultorios | CitaCloud")
@PermitAll
public class ConsultoriosView extends VerticalLayout {

    private final ConsultorioService consultorioService;
    private final SucursalService sucursalService;
    private final UUID empresaId;
    private final Grid<Consultorio> grid = new Grid<>(Consultorio.class, false);
    private final TextField codigoFiltro = new TextField("C\u00f3digo");
    private final TextField nombreFiltro = new TextField("Nombre");
    private final ComboBox<Sucursal> sucursalFiltro = new ComboBox<>("Sucursal");
    private final ComboBox<String> estadoFiltro = new ComboBox<>("Estado");

    public ConsultoriosView(ConsultorioService consultorioService, SucursalService sucursalService) {
        this.consultorioService = consultorioService;
        this.sucursalService = sucursalService;
        TenantUserDetails user = AuthService.getAuthenticatedUser();
        empresaId = user == null ? null : user.getEmpresaId();

        setSizeFull();
        setPadding(true);
        setSpacing(true);
        configurarFiltros();
        configurarTabla();
        add(crearEncabezado(), crearBarraFiltros(), grid);
        actualizarConsultorios();
    }

    private void configurarFiltros() {
        codigoFiltro.setPlaceholder("Buscar por c\u00f3digo");
        codigoFiltro.setPrefixComponent(VaadinIcon.SEARCH.create());
        nombreFiltro.setPlaceholder("Buscar por nombre");
        nombreFiltro.setPrefixComponent(VaadinIcon.SEARCH.create());
        sucursalFiltro.setPlaceholder("Seleccione sucursal");
        sucursalFiltro.setItemLabelGenerator(Sucursal::getNombre);
        estadoFiltro.setItems("Activo", "Inactivo");
        estadoFiltro.setPlaceholder("Seleccione estado");
        if (empresaId != null) {
            sucursalFiltro.setItems(sucursalService.listarPorEmpresa(empresaId));
        }
    }

    private HorizontalLayout crearEncabezado() {
        H2 titulo = new H2("Consultorios");
        titulo.getStyle().set("margin", "0").set("font-size", "1.5rem").set("font-weight", "800");
        Button nuevo = new Button(VaadinIcon.PLUS.create(), event -> abrirFormulario(null));
        nuevo.setTooltipText("Nuevo consultorio");
        nuevo.addThemeVariants(ButtonVariant.LUMO_PRIMARY);
        nuevo.getStyle().set("background-color", "#16a34a").set("color", "#ffffff");
        Button buscar = new Button(VaadinIcon.SEARCH.create(), event -> actualizarConsultorios());
        buscar.setTooltipText("Buscar");
        buscar.addThemeVariants(ButtonVariant.LUMO_PRIMARY);
        Button limpiar = new Button(VaadinIcon.ERASER.create(), event -> limpiarFiltros());
        limpiar.setTooltipText("Limpiar");
        limpiar.getStyle().set("background-color", "#e2e8f0").set("color", "#334155");
        HorizontalLayout acciones = new HorizontalLayout(nuevo, buscar, limpiar);
        acciones.setSpacing(false);
        acciones.getStyle().set("gap", "0.35rem");
        HorizontalLayout encabezado = new HorizontalLayout(titulo, acciones);
        encabezado.setWidthFull();
        encabezado.setJustifyContentMode(FlexComponent.JustifyContentMode.BETWEEN);
        encabezado.setAlignItems(FlexComponent.Alignment.CENTER);
        return encabezado;
    }

    private HorizontalLayout crearBarraFiltros() {
        HorizontalLayout filtros = new HorizontalLayout(codigoFiltro, nombreFiltro, sucursalFiltro, estadoFiltro);
        filtros.setWidthFull();
        filtros.setFlexGrow(1, codigoFiltro, nombreFiltro, sucursalFiltro, estadoFiltro);
        filtros.getStyle().set("flex-wrap", "wrap");
        filtros.getStyle().set("background-color", "#ffffff").set("padding", "1rem")
                .set("border-radius", "12px").set("border", "1px solid #e2e8f0");
        return filtros;
    }

    private void configurarTabla() {
        grid.addColumn(Consultorio::getCodigo).setHeader("C\u00d3DIGO").setWidth("140px").setFlexGrow(0);
        grid.addColumn(Consultorio::getNombre).setHeader("NOMBRE");
        grid.addColumn(consultorio -> consultorio.getSucursal() == null ? "" : consultorio.getSucursal().getNombre())
                .setHeader("SUCURSAL");
        grid.addColumn(Consultorio::getUbicacion).setHeader("UBICACI\u00d3N");
        grid.addComponentColumn(consultorio -> {
            Span estado = new Span(Boolean.TRUE.equals(consultorio.getActivo()) ? "Activo" : "Inactivo");
            estado.addClassName(Boolean.TRUE.equals(consultorio.getActivo()) ? "badge-activo" : "badge-inactivo");
            return estado;
        }).setHeader("ESTADO").setWidth("125px").setFlexGrow(0);
        grid.addComponentColumn(consultorio -> {
            Button editar = new Button(VaadinIcon.EDIT.create(), event -> abrirFormulario(consultorio));
            editar.setTooltipText("Editar");
            editar.addThemeVariants(ButtonVariant.LUMO_TERTIARY);
            return editar;
        }).setHeader("ACCIONES").setWidth("115px").setFlexGrow(0);
        grid.setWidthFull();
    }

    private void actualizarConsultorios() {
        if (empresaId == null) {
            grid.setItems(List.of());
            return;
        }
        Boolean activo = "Activo".equals(estadoFiltro.getValue()) ? Boolean.TRUE
                : "Inactivo".equals(estadoFiltro.getValue()) ? Boolean.FALSE : null;
        grid.setItems(consultorioService.buscar(empresaId, codigoFiltro.getValue(), nombreFiltro.getValue(),
                sucursalFiltro.getValue() == null ? null : sucursalFiltro.getValue().getId(), activo));
    }

    private void limpiarFiltros() {
        codigoFiltro.clear();
        nombreFiltro.clear();
        sucursalFiltro.clear();
        estadoFiltro.clear();
        actualizarConsultorios();
    }

    private void abrirFormulario(Consultorio existente) {
        if (empresaId == null) {
            Notification.show("No se pudo identificar la empresa de la sesi\u00f3n.", 3000, Notification.Position.MIDDLE);
            return;
        }
        boolean esEdicion = existente != null;
        Dialog dialog = new Dialog();
        dialog.setHeaderTitle(esEdicion ? "Editar Consultorio" : "Nuevo Consultorio");
        dialog.setWidth("600px");

        TextField codigo = new TextField("C\u00f3digo");
        codigo.setReadOnly(true);
        TextField nombre = new TextField("Nombre");
        nombre.setRequiredIndicatorVisible(true);
        nombre.setMaxLength(100);
        ComboBox<Sucursal> sucursal = new ComboBox<>("Sucursal");
        sucursal.setRequiredIndicatorVisible(true);
        sucursal.setItemLabelGenerator(Sucursal::getNombre);
        List<Sucursal> sucursales = new ArrayList<>(sucursalService.listarActivas(empresaId));
        if (esEdicion && existente.getSucursal() != null
                && sucursales.stream().noneMatch(item -> item.getId().equals(existente.getSucursal().getId()))) {
            sucursales.add(existente.getSucursal());
        }
        sucursal.setItems(sucursales);
        TextField ubicacion = new TextField("Ubicaci\u00f3n");
        ubicacion.setMaxLength(150);
        if (esEdicion) {
            codigo.setValue(existente.getCodigo());
            nombre.setValue(existente.getNombre());
            sucursal.setValue(existente.getSucursal());
            ubicacion.setValue(existente.getUbicacion() == null ? "" : existente.getUbicacion());
        }
        FormLayout formulario = esEdicion
                ? new FormLayout(codigo, nombre, sucursal, ubicacion)
                : new FormLayout(nombre, sucursal, ubicacion);
        formulario.setResponsiveSteps(new FormLayout.ResponsiveStep("0", 1), new FormLayout.ResponsiveStep("420px", 2));
        dialog.add(formulario);

        Button guardar = new Button(VaadinIcon.DISC.create(), event -> {
            try {
                if (esEdicion) {
                    consultorioService.actualizar(empresaId, existente.getId(), codigo.getValue(), nombre.getValue(),
                            sucursal.getValue() == null ? null : sucursal.getValue().getId(), ubicacion.getValue());
                } else {
                    consultorioService.crear(empresaId, nombre.getValue(),
                            sucursal.getValue() == null ? null : sucursal.getValue().getId(), ubicacion.getValue());
                }
                dialog.close();
                actualizarConsultorios();
                Notification.show(esEdicion ? "Consultorio actualizado." : "Consultorio creado.",
                        3000, Notification.Position.BOTTOM_START);
            } catch (IllegalArgumentException exception) {
                Notification.show(exception.getMessage(), 4000, Notification.Position.MIDDLE);
            }
        });
        guardar.setTooltipText("Guardar");
        guardar.addThemeVariants(ButtonVariant.LUMO_PRIMARY);
        guardar.getStyle().set("background-color", "#16a34a").set("color", "#ffffff");

        Button cerrar = new Button(VaadinIcon.CLOSE.create(), event -> dialog.close());
        cerrar.setTooltipText("Cerrar");
        cerrar.getStyle().set("background-color", "#e2e8f0").set("color", "#1e293b");
        dialog.getFooter().add(guardar);
        if (esEdicion) {
            boolean activo = Boolean.TRUE.equals(existente.getActivo());
            Button cambiarEstado = new Button(activo ? VaadinIcon.BAN.create() : VaadinIcon.CHECK.create(), event -> {
                try {
                    consultorioService.cambiarEstado(empresaId, existente.getId(), !activo);
                    dialog.close();
                    actualizarConsultorios();
                    Notification.show(activo ? "Consultorio desactivado." : "Consultorio activado.",
                            3000, Notification.Position.BOTTOM_START);
                } catch (IllegalArgumentException exception) {
                    Notification.show(exception.getMessage(), 4000, Notification.Position.MIDDLE);
                }
            });
            cambiarEstado.setTooltipText(activo ? "Desactivar consultorio" : "Activar consultorio");
            cambiarEstado.getStyle().set("background-color", activo ? "#dc2626" : "#16a34a").set("color", "#ffffff");
            dialog.getFooter().add(cambiarEstado);
        }
        dialog.getFooter().add(cerrar);
        dialog.open();
    }
}
