...
public class MiAgendaView extends VerticalLayout {
    ...
    private final DatePicker fecha = new DatePicker("Fecha"); private final TextField paciente = new TextField("Paciente"); private final ComboBox<String> estado = new ComboBox<>("Estado"); private final Grid<Cita> grid = new Grid<>(Cita.class, false); private final PaginadorTabla<Cita> paginador = new PaginadorTabla<>(grid);
    public MiAgendaView(CitaService citaService, MedicoRepository medicoRepository) {
        ...
        add(encabezado,filtro,grid,paginador); actualizar();
    }
    ...
    private void actualizar() { if(empresaId==null||usuarioId==null){paginador.setItems(List.of());return;} Medico medico=medicoRepository.findByEmpresaIdAndUsuarioId(empresaId,usuarioId).orElse(null); if(medico==null){paginador.setItems(List.of()); Notification.show("Tu usuario no est\u00e1 asociado a un m\u00e9dico.",3000,Notification.Position.MIDDLE);return;} String nombre=paciente.getValue()==null?"":paciente.getValue().trim().toLowerCase(java.util.Locale.ROOT); paginador.setItems(citaService.listarPorFecha(empresaId,fecha.getValue()).stream().filter(c->medico.getId().equals(c.getMedico().getId())).filter(c->nombre.isBlank() || c.getPaciente().getNombreCompleto().toLowerCase(java.util.Locale.ROOT).contains(nombre)).filter(c->"Todos".equals(estado.getValue()) || estado.getValue().equals(c.getEstado())).sorted(Comparator.comparing(Cita::getHoraInicio)).toList()); }
