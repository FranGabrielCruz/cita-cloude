...
public class ConfiguracionView extends VerticalLayout {
    ...
    private VerticalLayout crearNotificacionesCitas() {
        ...
        Span ayuda = new Span("Configure el correo automático enviado a los pacientes al crear una cita."); ayuda.getStyle().set("color", "var(--lumo-secondary-text-color)");
        ...
        boolean correoConfigurado = notificacionesCitas.correoConfigurado();
        Span estadoCorreo = new Span(correoConfigurado ? "Canal configurado" : "Canal no configurado"); estadoCorreo.addClassName(correoConfigurado ? "badge-activo" : "badge-inactivo");
        VerticalLayout canalCorreo = new VerticalLayout(correoActivo, estadoCorreo); canalCorreo.setPadding(false); canalCorreo.setSpacing(false);
        ...
        VerticalLayout seccionCorreo = tarjetaCanal("Correo electrónico", asunto, mensajeCorreo);
        ...
        etiquetas.forEach((variable, etiqueta) -> { Button chip = new Button(etiqueta); chip.addThemeVariants(ButtonVariant.LUMO_TERTIARY_INLINE, ButtonVariant.LUMO_SMALL); chip.addClassName("notification-variable-chip"); chip.setTooltipText("Insertar {{"+variable+"}}"); chip.addClickListener(e -> mensajeCorreo.getElement().executeJs("const el=this; const start=el.inputElement.selectionStart ?? el.value.length; const end=el.inputElement.selectionEnd ?? start; el.value=el.value.substring(0,start)+$0+el.value.substring(end); el.dispatchEvent(new Event('input',{bubbles:true})); el.inputElement.focus(); el.inputElement.setSelectionRange(start+$0.length,start+$0.length);", "{{"+variable+"}}")); variables.add(chip); });
        Button vistaPrevia = botonIcono(VaadinIcon.EYE.create(), "Vista previa", "transparent", "var(--lumo-primary-text-color)"); vistaPrevia.addClickListener(e -> abrirVistaPrevia(asunto.getValue(), mensajeCorreo.getValue()));
        Button guardar = botonIcono(VaadinIcon.DISC.create(), "Guardar", "#16a34a", "white"); guardar.addClickListener(e -> {try{notificacionesCitas.guardar(empresaId, correoActivo.getValue(), asunto.getValue(), mensajeCorreo.getValue());Notification.show("Configuración de notificaciones guardada.",3000,Notification.Position.BOTTOM_START);}catch(IllegalArgumentException ex){Notification.show(ex.getMessage(),4500,Notification.Position.MIDDLE);}});
        ...
        tarjeta.add(titulo, ayuda, canalCorreo, seccionCorreo, new Span("Variables disponibles"), variables, acciones); return tarjeta;
    }
