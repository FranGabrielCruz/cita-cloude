...
@Route(value = "check-in", layout = MainLayout.class)
@PageTitle("Check-in | CitaCloud")
@PermitAll
public class CheckInView extends VerticalLayout {
    public CheckInView(CitaService citas, CheckInService checkins) {
        TenantUserDetails u = AuthService.getAuthenticatedUser();
        UUID empresa = u == null ? null : u.getEmpresaId();
        setSizeFull();
        setPadding(true);
        add(new H2("Check-in"), new Paragraph("Registre la llegada de pacientes con cita confirmada para hoy."));
        Grid<Cita> grid = new Grid<>(Cita.class, false);
        grid.addColumn(c -> c.getPaciente().getNombreCompleto()).setHeader("PACIENTE");
        grid.addColumn(c -> c.getPaciente().getDocumento()).setHeader("DOCUMENTO");
        grid.addColumn(c -> c.getMedico().getNombreCompleto()).setHeader("MÉDICO");
        grid.addColumn(Cita::getHoraInicio).setHeader("HORA");
        grid.addComponentColumn(c -> {
            Button b = new Button("Registrar llegada", e -> {
                try {
                    checkins.registrar(empresa, u.getUsuarioId(), c.getId());
        ...
        grid.setItems(empresa == null ? List.of() : citas.listarPorFecha(empresa, LocalDate.now()).stream().filter(c -> "CONFIRMADA".equals(c.getEstado())).toList());
        grid.setWidthFull();
        add(grid);
    }
}
